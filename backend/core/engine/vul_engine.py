...
class VulEngine:
    ...
    def method_pool_signatures(self):
        ...
        for method in self.method_pool:
            signatures.add(
                f"{method['className'].replace('/', '.')}.{method.get('methodName')}"
            )
        return signatures
        ...
        for pool in self.method_pool:
            if "sourceType" in pool:
                ...
                vecs = (
                    ...
                    for s in reduce(
                        lambda x, y: x | y,
                        (target_hash_dict[i] for i in pool["sourceHash"]),
                        set(),
                    )
                )
            ...
            if nk.has_path(g, s, t):
                ...
                if (
                    ...
                    and len(
                        list(
                            filter(
                                lambda x: "ranges" in x and x["ranges"],
                                invokeid_dict[total_path[-2]]["targetRange"],
                            )
                        )
                    )
                    == 0
        ...
        for total_path in final_path:
            ...
            for path_key in total_path:
                ...
                if sub_method.get("source"):
                    ...
                    if self.version == 3:
                        self.taint_value = parse_target_value(
                            sub_method["targetValues"]
                        )
                    else:
    ...
    def vul_filter(self):
        # Analyze whether the filter conditions exist and eliminate false positives
        # Query the filter method based on the vulnerability type
        # Check vul_
    ...
    def copy_method(
        ...
        # todo  Splicing by type
        if source:
            vul_method_detail["tag"] = "source"
            vul_method_detail["code"] = (
    ...
    def dfs(self, current_hash, left_node, left_index):
        """
        Depth-first search, searching for edges in the taint flow graph.
        :param current_hash: Current taint data, set()
        :param left_node: The call ID of the parent node method
        :param left_index: The index of the parent node method in the method queue
        """
        not_found = True
