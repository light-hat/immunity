...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        POLICY_DIR = os.path.join(BASE_DIR, "ruleset/")
        ...
        for strategy in tqdm(full_strategies, desc="strategy"):
            if IastStrategyModel.objects.filter(
                ...
                # The policy type already exists and will not be recreated.
            ...
            if (
                ...
                # There are user-defined conflicting policies that will not be modified
                continue
        ...
        for k, v in LANGUAGE_DICT.items():
            ...
            for hook_type in tqdm(hooktypes, desc="hook_type"):
                if HookType.objects.filter(
                    ...
                    # The policy type already exists and will not be rebuilt. New rules will be added to it.
                    ...
                    for hook_strategy in policy["details"]:
                        if HookStrategy.objects.filter(
                            ...
                            hook_strategy_obj = HookStrategy.objects.filter(
                                ...
                                type=hook_strategy["type"],
                                language_id=v,
                                enable__in=(0, 1),  # Compatible with fake delete
                            ).get()
                            hook_strategy_obj.system_type = 1
                            hook_strategy_obj.modified = True
                            hook_strategy_obj.save()
