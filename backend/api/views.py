...
def import_preferences(request):
    ...
    try:
        ...
        logger.error("Failed to import preferences for user %s", request.user.username, exc_info=True)
        return Response({
            'success': False,
    ...
    try:
        ...
        if refresh_token:
            try:
                ...
                logger.warning(f"Failed to blacklist token for user {request.user.username}: {token_error}")
        else:
            logger.info(f"User {request.user.username} logged out - no refresh token found in cookies")
