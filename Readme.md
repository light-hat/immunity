...
- контекст выполнения запроса (связка: `запрос + поток управления + ответ`) на наличие CWE (9 уязвимостей);
- анализ зависимостей (`версии используемых библиотек`) на наличие CVE, PYSEC и т.д. по открытым базам;
- анализ конфигурации (отдельный от ML плагин для обработки `настроек проекта`).
